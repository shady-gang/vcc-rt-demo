...
RA_FUNCTION vec3 pathtrace_ao(RNGState* rng, BVH& bvh, Ray ray) {
    ...
    if (bvh.intersect(ray, hit)) {
        ...
        auto sample_dir = shading::sample_cosine_hemisphere(randf(rng), randf(rng));

        Ray bounced_ray = { .origin = p };
        bounced_ray.origin = p;
        bounced_ray.dir = shading::to_world(sample_dir.dir, frame);
        ...
        if (bvh.intersect(bounced_ray, hit))
            ...
            return vec3(fabs(sample_dir.dir.z/*cosine*/)) / sample_dir.pdf;
    } else {
        return vec3(1);
    }
}